...
function setup() {
  ...
  warning = createSprite(displayWidth/2 , runner.y - 200, 250 , 100);
  warning.addImage('sign' , warningImg);
  warning.scale = 0.25;
...
function collectingGarbage(){
 ...
 for (var i = 0; i < bioWasteGroup.length; i++) {
	...
	if (bioWasteGroup.get(i).isTouching(runner) && keyDown('B')) {
...
for (var i = 0; i < nonBioWasteGroup.length; i++) {
	...
	if (nonBioWasteGroup.get(i).isTouching(runner) && keyDown('N')) {
...
for (var i = 0; i < eWasteGroup.length; i++) {
	...
	if (eWasteGroup.get(i).isTouching(runner) && keyDown('V')) {
